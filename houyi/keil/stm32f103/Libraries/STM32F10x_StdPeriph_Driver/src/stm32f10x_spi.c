...
/**
  * @brief  Configures the baud rate for the selected SPI.
  * @param  SPIx: where x can be 1, 2 or 3 to select the SPI peripheral.
  * @param  SPI_BaudRate: specifies the SPI baud rate.
  * @retval None
  */
void SPI_BaudRateConfig(SPI_TypeDef* SPIx, uint16_t SPI_BaudRate)
{
  /* Check the parameters */
  assert_param(IS_SPI_ALL_PERIPH(SPIx));
  assert_param(IS_SPI_BAUDRATE_PRESCALER(SPI_BaudRate));
  /* Clear BR */
  SPIx->CR1 &= (uint16_t)~SPI_BaudRatePrescaler_256;
  /* Set new BR value */
  SPIx->CR1 |= SPI_BaudRate;
}
